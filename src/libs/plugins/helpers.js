...
export const useDocument = () => {
  const name = process.env.REACT_APP_DOMAIN_AREA_NAME;
  const company = `Pifa ${name}`;
  const short = `pifa-${toPathname(name)}`;
  const domain = process.env.REACT_APP_DOMAIN_AREA;
